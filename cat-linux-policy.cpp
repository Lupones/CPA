										   ...
										   uint64_t num_critical_old, uint64_t num_critical_new)
{
	...
	num_shared_ways = 2;
	LOGINF("[UPDATE] CLOS 1 (non-CR) has mask {:#x} ({} ways)"_format(
			LinuxBase::get_cat()->get_cbm(1), num_ways_CLOS_1));
	LOGINF("[UPDATE] CLOS 2 (CR) has mask {:#x} ({} ways)"_format(LinuxBase::get_cat()->get_cbm(2),
																  num_ways_CLOS_2));
...
void CriticalPhaseAware::divide_3_critical(uint64_t clos, bool limitDone)
{
	...
	LOGINF("[LLC] CLOS 2 now has mask {:#x} ({} ways)"_format(maskCLOS2, num_ways_CLOS_2));
	LOGINF("[LLC] CLOS 3 now has mask {:#x} ({} ways)"_format(maskCLOS3, num_ways_CLOS_3));
	LOGINF("[LLC] CLOS 4 now has mask {:#x} ({} ways)"_format(maskCLOS4, num_ways_CLOS_4));
...
void CriticalPhaseAware::divide_1_critical(uint64_t clos)
{
	...
	LOGINF("[LLC] CLOS 1 now has mask {:#x} ({} ways)"_format(maskNonCrCLOS, num_ways_CLOS_1));
	LOGINF("[LLC] CLOS 2 now has mask {:#x} ({} ways)"_format(maskCLOS2, num_ways_CLOS_2));
	LOGINF("[LLC] CLOS 3 now has mask {:#x} ({} ways)"_format(maskCLOS3, num_ways_CLOS_3));
	LOGINF("[LLC] CLOS 4 now has mask {:#x} ({} ways)"_format(maskCLOS4, num_ways_CLOS_4));
}
...
void CriticalPhaseAware::apply(uint64_t current_interval, const tasklist_t &tasklist)
{
			...
			if (ipc_ICOV >= icov)
			{
					...
					(CLOSvalue <= 4))
				{
					...
					if (critical_apps == 1)
					{
						...
						LOGINF("CLOS 1 now has mask {:#x} ({} ways)"_format(maskNonCrCLOS,
																			num_ways_CLOS_1));
						LOGINF("CLOS 2 now has mask {:#x} ({} ways)"_format(maskCLOS2,
																			num_ways_CLOS_2));
						LOGINF("CLOS 3 now has mask {:#x} ({} ways)"_format(maskCLOS3,
																			num_ways_CLOS_3));
						LOGINF("CLOS 4 now has mask {:#x} ({} ways)"_format(maskCLOS4,
																			num_ways_CLOS_4));
					}
	...
	for (auto const &x : id_phase_change)
	{
		...
		switch (CLOSvalue)
		{
			case 1:																   // Non-critical
				if ((MPKIL3Task >= 10) & (HPKIL3Task >= 10) & (IPCTask <= ipcLow)) // 1. BULLY
				{
					...
					countCLOS = task_increase_clos_change_count(*taskCLOS);
						 ...
						 (HPKIL3Task < hpkil3Limit)) // 2. SQUANDERER
				{
				...
				{
					if ((MPKIL3Task >= limit_outlier) & (HPKIL3Task >= hpkil3Limit) &
						(IPCTask <= ipcMedium)) // 3. CRITICAL
					{
						LOGINF("The MPKI_L3 of task {} is an outlier, since MPKIL3 {} >= {} & HPKIL3 {} >= {}"_format(
					...
					else if ((l3_occup_mb > limit_space) & (HPKIL3Task < 0.5) &
							 (MPKIL3Task < 0.5)) // 4. GREEDY
					{
						LOGINF("[TEST] {}: has l3_occup_mb {} -> isolate!"_format(taskID,
																				  l3_occup_mb));
						if (n_isolated_apps < 2)
			...
			case 3:
			case 4: // Critical
				if ((HPKIL3Task > MPKIL3Task) &
					(MPKIL3Task < limit_outlier)) // 1. PROFITABLE CRITICAL
				{
				...
				else if ((MPKIL3Task >= 10) & (HPKIL3Task >= 10) & (IPCTask <= ipcLow)) // 2. BULLY
				{
					...
					countCLOS = task_increase_clos_change_count(*taskCLOS);
						 ...
						 (HPKIL3Task >= hpkil3Limit)) // 3. STILL CRITICAL
				{
					LOGINF("Task {} is still critical!"_format(taskID));
					outlier.push_back(std::make_pair(taskID, 1));
				}
				else if ((MPKIL3Task >= limit_outlier) &
						 (HPKIL3Task < hpkil3Limit)) // 4. SQUANDERER
				{
					LOGINF("The MPKI_L3 of task {} is an outlier but HPKIL3 is very low {}!! -> SQUANDERER"_format(
							taskID, HPKIL3Task));
					if (n_isolated_apps < 2)
			...
			case 6: // Greedy or squanderer
				if ((MPKIL3Task >= 10) & (HPKIL3Task >= 10) & (IPCTask <= ipcLow)) // 1. BULLY
				{
					...
					countCLOS = task_increase_clos_change_count(*taskCLOS);
						 ...
						 (HPKIL3Task < hpkil3Limit)) // 3. SQUANDERER
				{
	...
	for (auto const &x : taskIsInCRCLOS)
	{
		...
		switch (CLOSvalue)
		{
			...
			case 4:
				if (it1 == outlier.end())
					outlier.push_back(std::make_pair(taskID, 1));
				LOGINF("[LLC] Task {} CLOS {} addded to LLCoccup_critical"_format(taskID,
																				  CLOSvalue));
	...
	LOGINF("Total LLCoccup_critical = {}"_format(LLC_critical));
		...
		else
		{
			if (critical_apps == 1)
			{
				...
				auto x = std::max_element(
						LLCoccup_critical.begin(), LLCoccup_critical.end(),
						[](const std::pair<int, int> &p1, const std::pair<int, int> &p2) {
							return p1.second < p2.second;
						});
				uint32_t maxID = x->first;

				auto it = std::find_if(v_ipc.begin(), v_ipc.end(), [&maxID](const auto &tuple) {
					return std::get<0>(tuple) == maxID;
				});
				...
				if ((maxIPC >= ipcMedium) & (limit_task[maxID] == false))
				{
					...
					auto it2 = std::find_if(
							taskIsInCRCLOS.begin(), taskIsInCRCLOS.end(),
							[&maxID](const auto &tuple) { return std::get<0>(tuple) == maxID; });
					uint64_t CLOSvalue = std::get<1>(*it2);
				...
				for (const auto &myPair : LLCoccup_critical)
				{
					...
					if ((LLC_critical/2) <= occup) {
						...
						if (ipcTask >= ipcMedium)
						{
							...
							else if ((critical_apps == 3) & (limit == true) & (limit_task[taskID] == false))
								divide_2_critical(CLOSvalue); // 2/3
							else if ((critical_apps == 3) & (limit == false) & (limit_task[taskID] == false))
			...
			else if ((change_critical == false) & (critical_apps > 0) & (critical_apps < 4))
			{
				...
				LOGINF("CLOS 1 (non-CR) has mask {:#x} ({} ways)"_format(
						LinuxBase::get_cat()->get_cbm(1), num_ways_CLOS_1));
				LOGINF("CLOS 2 (CR)     has mask {:#x} ({} ways)"_format(
						LinuxBase::get_cat()->get_cbm(2), num_ways_CLOS_2));
				if (critical_apps > 1)
					LOGINF("CLOS 3 (CR)     has mask {:#x} ({} ways)"_format(
							LinuxBase::get_cat()->get_cbm(3), num_ways_CLOS_3));
				if (critical_apps > 2)
					LOGINF("CLOS 4 (CR)     has mask {:#x} ({} ways)"_format(
							LinuxBase::get_cat()->get_cbm(4), num_ways_CLOS_4));
 ...
 * number of critical apps is detected
 */
void CriticalAwareV2::update_configuration(std::vector<pair_t> v, std::vector<pair_t> status,
										   uint64_t num_critical_old, uint64_t num_critical_new)
{
...
void CriticalAwareV2::apply(uint64_t current_interval, const tasklist_t &tasklist)
{
	...
	for (const auto &task_ptr : tasklist)
	{
		...
		LOGINF("Task {} ({}): IPC {}, MPKIL3 {}, HPKIL3 {}, APKIL3 {}, l3_occup_mb {}"_format(
				taskName, taskID, ipc, MPKIL3, HPKIL3, APKIL3, l3_occup_mb));
		...
		if (it2 != valid_mpkil3.end())
		{
			...
			auto it_pm = std::find_if(
					v_mpkil3_prev.begin(), v_mpkil3_prev.end(),
					[&taskID](const auto &tuple) { return std::get<0>(tuple) == taskID; });
			...
			else if (itX == id_isolated.end())
			{
				...
				if (it3 != taskIsInCRCLOS.end())
				{
					...
					if ((l3_occup_mb > 3) & (CLOSvalue == 1) & (HPKIL3 < 1))
					{
						...
						LOGINF("[TEST] {}: has l3_occup_mb {} -> assigned to CLOS {}"_format(
								taskID, l3_occup_mb, CLOS_isolated));
						LinuxBase::get_cat()->set_cbm(CLOS_isolated, mask_isolated);
						LOGINF("[TEST] CLOS {} has now mask {:x}"_format(CLOS_isolated,
																		 mask_isolated));
			...
			if (((phase_count[taskID] == 1) & (phase_duration[taskID] >= windowSizeM[taskID])) |
				((phase_count[taskID] > 1) & (phase_duration[taskID] > 1)))
			{
				// Calculate ICOV
				...
				if (my_ICOV >= 0.5)
				{
					LOGINF("{}: NEW PHASE {} COMMING. Prev phase duration: {}"_format(
							taskID, phase_count[taskID], phase_duration[taskID]));
					sumXij[taskID] = 0;
					...
					deque_valid.clear();
					LOGINF("{}: deque_valid has been cleared as a new phase is starting."_format(
							taskID));
				}
			}
	...
	for (auto const &x : valid_mpkil3)
	{
		...
		{
			LOGINF("RESET -> Task {} has been CRITICAL THEREFORE ITS VALUES ARE NOT CONSIDERED"_format(
					idTask));
			for (auto i = val.cbegin(); i != val.cend(); ++i)
				res = res + std::to_string(*i) + " ";
		}
	...
	else
	{
		...
		auto itlim = std::find_if(v_limits.begin(), v_limits.end(), [&outl](const auto &tuple) {
			return std::get<0>(tuple) == outl;
		});
	...
	const double max =
			std::get<1>(*std::max_element(v_mpkil3.begin(), v_mpkil3.end(), less_by_second));
	...
	for (const auto &item : v_mpkil3)
	{
		...
		if (MPKIL3Task >= limit_outlier)
		{
			LOGINF("The MPKI_L3 of task with id {} is an outlier, since {} >= {}"_format(
					idTask, MPKIL3Task, limit_outlier));
		...
		{
			LOGINF("The MPKI_L3 of task with id {} is NOT an outlier, since   {} < {}"_format(
					idTask, MPKIL3Task, limit_outlier));
			// LOGINF("Fraction critical of {} is {} --> CRITICAL"_format(idTask,
			// fractionCritical));
			LOGINF("HPKI_L3 of {} is {} (> {}) --> CRITICAL"_format(idTask, hpkil3Task,
																	hpkiL3Total / 3));
	...
	if (firstTime)
	{
		...
		for (const auto &item : outlier)
		{
			idTask = std::get<0>(item);
			uint32_t outlierValue = std::get<1>(item);
			auto it = std::find_if(v_ipc.begin(), v_ipc.end(), [&idTask](const auto &tuple) {
				return std::get<0>(tuple) == idTask;
			});
			double ipcTask = std::get<1>(*it);
			auto it1 = std::find_if(id_pid.begin(), id_pid.end(), [&idTask](const auto &tuple) {
	...
	else
	{
		...
		for (const auto &item : outlier)
		{
			...
			auto itW = std::find_if(v_ipc.begin(), v_ipc.end(), [&idTask](const auto &tuple) {
				return std::get<0>(tuple) == idTask;
			});
		...
		else
		{
			...
			if ((critical_apps > 0) && (critical_apps < 4))
			{
				...
				/*for (const auto &item : outlier)
				{
					...
					// Find L3 Occupancy
					auto itT = std::find_if(v_l3_occup_mb.begin(),
				v_l3_occup_mb.end(),[&idTask](const auto& tuple) {return std::get<0>(tuple) ==
				idTask;});
					double l3_occup_mb_task = std::get<1>(*itT);

					// Find HPKI-L3
					auto itH = std::find_if(v_hpkil3.begin(), v_hpkil3.end(),[&idTask](const  auto&
				tuple) {return std::get<0>(tuple) == idTask;});
					double hpkil3Task = std::get<1>(*itH);

					if (!outlierValue & (CLOS_isolated <= 3))
					{
						auto it2 = std::find_if(taskIsInCRCLOS.begin(),
				taskIsInCRCLOS.end(),[&idTask](const auto& tuple) {return std::get<0>(tuple) ==
				idTask;});
						uint64_t CLOSvalue = std::get<1>(*it2);
						assert((CLOSvalue >= 1) && (CLOSvalue <= 3));

						// Check if there is a non-critical application occupying more space than it
				should
						if ((l3_occup_mb_task > 3) & (CLOSvalue == 1) & (hpkil3Task < 1))
						{
							// Isolate it in a separate CLOS with two exclusive ways
							n_isolated_apps = n_isolated_apps + 1;
							auto it1 = std::find_if(id_pid.begin(), id_pid.end(),[&idTask](const
				auto& tuple) {return std::get<0>(tuple)  == idTask;});
							pid_t pidTask = std::get<1>(*it1);

							LinuxBase::get_cat()->add_task(CLOS_isolated,pidTask);
							LOGINF("[TEST] {}: has l3_occup_mb {} -> assigned to CLOS
				{}"_format(idTask,l3_occup_mb_task,CLOS_isolated));
				...
				/*for ( itc = ipc_critical_prev.begin(); itc != ipc_critical_prev.end(); itc++ )
				{
					...
					auto itW = std::find_if(v_ipc.begin(), v_ipc.end(),[&pidTask](const  auto&
				tuple) {return std::get<0>(tuple) == pidTask;});
					double ipcTask = std::get<1>(*itW);
					...
					if((!false_critical_app) && (ipcTask < 1.3*ipc_prev))
					{
						...
						// Update taskIsInCRCLOS
						auto it2 = std::find_if(taskIsInCRCLOS.begin(),
				taskIsInCRCLOS.end(),[&pidTask](const auto& tuple) {return std::get<0>(tuple)  ==
				pidTask;});
						it2 = taskIsInCRCLOS.erase(it2);
				...
				tuple) {return std::get<0>(tuple) == pidTask;});
					...
					if(ipcEXcl < 0.7*excluded_application_ipc)
						LOGINF("XX Isolated app {} in CLOS {} is BAD ({} compared to
				{})"_format(excluded_application,c,ipcEXcl,excluded_application_ipc));
					else
						LOGINF("XX Isolated app {} in CLOS {} is GOOD ({} compared to
				{})"_format(excluded_application,c,ipcEXcl,excluded_application_ipc));
